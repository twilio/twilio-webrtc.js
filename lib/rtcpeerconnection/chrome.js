...
function ChromeRTCPeerConnection(configuration) {
  ...
  Object.defineProperties(this, {
    ...
    _syncStreamsToContainerStreams: {
      value: new Map()
    },
    _tracksToSSRCs: {
      value: new Map()
    },
    _tracksToSyncStreams: {
      value: new Map()
    },
    localDescription: {
      enumerable: true,
...
ChromeRTCPeerConnection.prototype.addTrack = function addTrack() {
  ...
  if ('addTrack' in PeerConnection.prototype) {
    return this._peerConnection.addTrack.apply(this._peerConnection, args);
  }
  var mediaStreamTrack = args[0];
  var syncMediaStreams = args.slice(1);

  if (this._peerConnection.signalingState === 'closed') {
    throw new Error('Cannot add MediaStreamTrack [' + mediaStreamTrack.id + ', '
      + mediaStreamTrack.kind + ']: RTCPeerConnection is closed');
  }
  if (this._tracksToSyncStreams.has(mediaStreamTrack.id)) {
    throw new Error('Cannot add MediaStreamTrack [' + mediaStreamTrack.id + ', '
      + mediaStreamTrack.kind + ']: RTCPeerConnection already has it');
  }

  var syncMediaStreamIds = new Set(syncMediaStreams.map(function(mediaStream) {
    return mediaStream.id;
  }));
  this._tracksToSyncStreams.set(mediaStreamTrack.id, syncMediaStreamIds);

  syncMediaStreamIds.forEach(function(mediaStreamId) {
    var containerMediaStream = this._syncStreamsToContainerStreams.get(mediaStreamId);
    if (containerMediaStream) {
      this._peerConnection.removeStream(containerMediaStream);
    } else {
      containerMediaStream = new MediaStream();
    }
    containerMediaStream.addTrack(mediaStreamTrack);
    this._peerConnection.addStream(containerMediaStream);
    this._syncStreamsToContainerStreams.set(mediaStreamId, containerMediaStream);
  }, this);
...
ChromeRTCPeerConnection.prototype.removeTrack = function removeTrack(sender) {
  if ('removeTrack' in PeerConnection.prototype) {
    this._peerConnection.removeTrack(sender);
    return;
  }

  var mediaStreamTrack = sender.track;
  if (this._peerConnection.signalingState === 'closed') {
    throw new Error('Cannot remove MediaStreamTrack [' + mediaStreamTrack.id +
      ', ' + mediaStreamTrack.kind + ']: RTCPeerConnection is closed');
  }
  if (!this._tracksToSyncStreams.has(mediaStreamTrack.id)) {
    return;
  }
  var syncMediaStreamIds = this._tracksToSyncStreams.get(mediaStreamTrack.id);
  this._tracksToSyncStreams.delete(mediaStreamTrack.id);

  syncMediaStreamIds.forEach(function(mediaStreamId) {
    var containerMediaStream = this._syncStreamsToContainerStreams.get(mediaStreamId);
    this._peerConnection.removeStream(containerMediaStream);
    containerMediaStream.removeTrack(mediaStreamTrack);
    if (containerMediaStream.getTracks().length > 0) {
      this._peerConnection.addStream(containerMediaStream);
    }
  }, this);
};
...
ChromeRTCPeerConnection.prototype.getSenders = function getSenders() {
  if ('getSenders' in PeerConnection.prototype) {
    return this._peerConnection.getSenders();
  }
  // A set is created here to avoid duplicates.
  var mediaStreamTracks = new Set(util.flatMap(this._peerConnection.getLocalStreams(), function(mediaStream) {
    return mediaStream.getTracks();
  }));
  return Array.from(mediaStreamTracks.values()).map(function(mediaStreamTrack) {
    return { track: mediaStreamTrack };
  });
};
