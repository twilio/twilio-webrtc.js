...
SafariRTCPeerConnection.prototype.addTrack = function addTrack() {
  ...
  sender = getActiveSenders(this._peerConnection).get(track.id)
    || this._peerConnection.addTrack.apply(this._peerConnection, args);

  sender = sender || getActiveSenders(this._peerConnection).get(track.id);
...
SafariRTCPeerConnection.prototype.removeTrack = function removeTrack(sender) {
  ...
  sender = this._senders.get(track.id);
  if (sender && sender.track === track) {
    this._senders.set(track.id, new RTCRtpSenderShim(null));
  }
};
