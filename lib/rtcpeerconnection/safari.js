...
// return it.
//
// UPDATE(mmalavalli): Because we are not delegating to the native
...
SafariRTCPeerConnection.prototype.addTrack = function addTrack() {
  ...
  if (this._senders.has(track.id)) {
    throw new Error('Cannot add MediaStreamTrack [' + track.id + ', '
      + track.kind + ']: RTCPeerConnection already has it');
  }
  var sender = getNativeSenders(this._peerConnection).get(track.id)
    || this._peerConnection.addTrack.apply(this._peerConnection, args);

  sender = sender || getNativeSenders(this._peerConnection).get(track.id);
  this._senders.set(track.id, sender);
  return sender;
};
...
SafariRTCPeerConnection.prototype.removeTrack = function removeTrack(sender) {
  ...
  if (this._isClosed) {
    throw new Error('Cannot remove MediaStreamTrack [' + track.id +
      ', ' + track.kind + ']: RTCPeerConnection is closed');
  }
  this._senders.delete(track.id);
};
...
/**
 * Gets the native RTCRtpSenders with non-null tracks.
 * @param peerConnection
 * @returns {Map<Track.ID, RTCRtpSender>}
 */
function getNativeSenders(peerConnection) {
